...
public class PlayerMovement : MonoBehaviour
{
    ...
    {
        if (!isAlive) { return; }
        if (!feetCollider.IsTouchingLayers(LayerMask.GetMask("Ground"))) // 점프 2번 못하게 막음
        {
            return;
        }
    ...
    void ClimbLadder()
    {
        ...
        {
            rigidbody.gravityScale = gravityScaleAtStart; // 사다리를 안밟고 있으면 gravity를 원래대로
        ...
        rigidbody.gravityScale = 0; // 사다리에서 점점 내려오는 현상 막음
